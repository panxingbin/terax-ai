...
export function ThemesSection() {
  ...
  const backgroundImageUrl = usePreferencesStore((s) => s.backgroundImageUrl);
  ...
  const onRemoveBackground = async () => {
    ...
    await setBackgroundImageUrl(null);
  ...
  const handleSubmitUrl = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
        setBgError("Only http or https URLs are supported.");
        return;
      }
    } catch {
      setBgError("That doesn't look like a valid URL.");
      return;
    }
    await setBackgroundImageUrl(trimmed);
    await setBackgroundKind("url");
    setUrlOpen(false);
  };
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {backgroundKind !== "none" && (backgroundImageId || backgroundImageUrl) ? (
              <Button
                variant="ghost"
                size="sm"
        ...
        {backgroundKind !== "none" && (backgroundImageId || backgroundImageUrl) ? (
          ...
          <p className="text-[11px] text-muted-foreground">
            Drop an image here, pick one, or paste a URL. Stored locally; doesn't
