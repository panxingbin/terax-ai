...
export type ShellEditorOptions = {
  ...
  /** Subcommand completion: given program + args, return candidate next tokens. */
  getSubcommands?: (program: string, args: string[]) => Promise<string[]>;
  ...
  /** AI command generation: given natural language, return a shell command. */
  onAiGenerate?: (prompt: string) => Promise<string | null>;
  /** Recent command history for AI context. */
  getHistory?: () => string[];
};
...
function makeCompletionSource(
  ...
  return async (ctx: CompletionContext): Promise<CompletionResult | null> => {
    ...
    // Subcommand completion: if we have a program and a subcommand fetcher
    if (getSubcommands && word.from > line.from) {
      const beforeText = line.text.slice(0, word.from - line.from);
      const parts = beforeText.trim().split(/\s+/).filter(Boolean);
      if (parts.length >= 1) {
        const program = parts[0];
        const args = parts.slice(1);
        try {
          const subs = await getSubcommands(program, args);
          if (subs.length > 0) {
            return {
              from: word.from,
              options: subs.map((s) => ({
