...
pub fn build_command(
    ...
    if let WorkspaceEnv::Remote { host, port, user, auth } = workspace {
        return build_remote(host, port, user, auth);
    }
    #[cfg(unix)]
    {
        let _ = workspace;
...
fn build_remote(
    ...
    // Force PTY allocation so interactive shells work (-t), disable SSH config (-F) for predictable behavior,
    // disable password auth prompts (-o BatchMode=no but allow agent/key), and ignore known_hosts for first connect.
    cmd.arg("-t");
