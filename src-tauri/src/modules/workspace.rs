...
fn ssh_hosts_path() -> Result<PathBuf, String> {
    let base = dirs::config_dir().ok_or_else(|| "config dir unavailable".to_string())?;
    let dir = base.join("terax");
    std::fs::create_dir_all(&dir).map_err(|e| format!("create config dir: {e}"))?;
    Ok(dir.join("ssh-hosts.json"))
}

fn load_ssh_hosts() -> Vec<SshHost> {
    let Ok(path) = ssh_hosts_path() else {
        return Vec::new();
    };
...
fn save_ssh_hosts(hosts: &[SshHost]) -> Result<(), String> {
    let path = ssh_hosts_path()?;
    let json = serde_json::to_vec_pretty(hosts).map_err(|e| format!("serialize: {e}"))?;
    let mut f = std::fs::File::create(&path).map_err(|e| format!("create: {e}"))?;
    f.write_all(&json).map_err(|e| format!("write: {e}"))?;
    Ok(())
}
...
#[tauri::command]
pub fn ssh_save_host(host: SshHost) -> Result<(), String> {
    let mut hosts = load_ssh_hosts();
    if let Some(slot) = hosts.iter_mut().find(|h| h.id == host.id) {
        *slot = host;
    } else {
        hosts.push(host);
    }
    save_ssh_hosts(&hosts)
}

#[tauri::command]
pub fn ssh_delete_host(id: String) -> Result<(), String> {
    let mut hosts = load_ssh_hosts();
...
#[tauri::command]
pub fn ssh_test_connection(host: SshHost) -> Result<(), String> {
...
#[tauri::command]
pub fn ssh_import_from_config() -> Result<usize, String> {
    let Some(home) = dirs::home_dir() else {
        return Err("home dir unavailable".to_string());
    };
    ...
    for line in content.lines() {
        ...
        let existing = content[..content.len() - content.trim_start().len()]
            .lines()
            .collect::<Vec<_>>();
        ...
        for (i, bl) in existing.iter().enumerate() {
            if bl.trim().eq_ignore_ascii_case(&format!("host {pattern}")) {
                for next in existing.iter().skip(i + 1) {
                    let t = next.trim();
                    if t.starts_with("host ") || t.starts_with("Host ") {
                        break;
                    }
                    if let Some(v) = t.strip_prefix("hostname ").or_else(|| t.strip_prefix("Hostname ")) {
                        hostname = Some(v.trim().to_string());
                    } else if let Some(v) = t.strip_prefix("user ").or_else(|| t.strip_prefix("User ")) {
                        user = Some(v.trim().to_string());
                    } else if let Some(v) = t.strip_prefix("port ").or_else(|| t.strip_prefix("Port ")) {
                        if let Ok(p) = v.trim().parse::<u16>() {
                            port = Some(p);
                        }
                    } else if let Some(v) = t.strip_prefix("identityfile ").or_else(|| t.strip_prefix("IdentityFile ")) {
                        identity = Some(v.trim().trim_matches('"').to_string());
                    }
                }
                break;
            }
        }
        let Some(target_host) = hostname.filter(|h| !h.is_empty()) else {
            continue;
        };
        if hosts.iter().any(|h| h.host == target_host && h.port == port.unwrap_or(22)) {
            continue;
        }
        let id = format!("ssh-{}", target_host.replace('.', "-"));
        hosts.push(SshHost {
            id,
            ...
            auth: identity
                .map(|p| SshAuth::Key { path: p, passphrase: None })
                .unwrap_or_default(),
        });
        imported += 1;
    }
