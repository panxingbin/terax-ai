...
// Multi-source suggestion: returns top-N prefix-extending history commands,
// optionally boosted by cwd match.
pub fn suggest_multi(index: &[HistEntry], line: &str, cwd: Option<&str>, limit: usize) -> Vec<String> {
    if line.is_empty() {
        return Vec::new();
    }
    ...
    let mut out: Vec<String> = Vec::new();
    for e in index {
        if e.cmd.len() > line.len() && e.cmd.starts_with(line) {
...
// Subcommand completion: from history, find commands that start with `program`
// and extract their next token as a subcommand suggestion.
pub fn subcommand_completions(index: &[HistEntry], program: &str, args: &[String]) -> Vec<String> {
    use std::collections::HashSet;
    let prefix = format!("{} {}", program, args.join(" "));
    let mut subs: HashSet<String> = HashSet::new();
    for e in index {
        if e.cmd.starts_with(&prefix) {
            let rest = &e.cmd[prefix.len()..];
